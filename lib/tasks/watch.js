...
module.exports = function (gulp, $, config) {
     ...
     * * `paths.app.scripts` the {@link lint lint:scripts}, {@link tests tests:run} tasks are executed. If the change is a new or removed or renamed file, the {@link inject} task is executed additionally.
     * * `paths.tests`, the {@link lint lint:tests} and {@link tests tests:run} task is run.
    ...
    gulp.task('watch', function () {
        ...
        gulp.watch(config.paths.app.scripts, ['tests:run']);

        // Run and lint tests.
        gulp.watch(config.paths.tests, ['tests:run']);
